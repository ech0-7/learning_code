...
class MotionBasisComputer(nn.Module):
    ...
    def forward(self, dst_Rs, dst_Ts, cnl_gtfms, tjoints):
        ...
        joints_homogen = torch.cat([tjoints[..., None], torch.zeros([batch_size, TOTAL_BONES, 1, 1], dtype=dst_gtfms.dtype, device=dst_gtfms.device)],dim=2)
        init_bone = torch.matmul(dst_gtfms, joints_homogen)
