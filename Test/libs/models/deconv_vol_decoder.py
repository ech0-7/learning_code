...
class MotionWeightVolumeDecoder(nn.Module):
    def __init__(self, embedding_size=256, volume_size=64, total_bones=24):
    ...
    def forward(self, motion_weights_priors): # 24+1, 32, 32, 32)
        embedding = self.const_embedding[None, ...] # (1, 256)
        decoded_weights =  F.softmax(self.decoder(embedding) + \
                                        torch.log(motion_weights_priors.reshape(1,25,self.volume_size,self.volume_size,self.volume_size)).to(self.decoder(embedding).device), 
                                     dim=1)
        
        return decoded_weights#(1,25,64,64,64)
