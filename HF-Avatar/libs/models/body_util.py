...
def _deform_gaussian_volume(
    ...
    bbox_min_xyz = bbox_min_xyz.cpu().numpy()
    bbox_max_xyz = bbox_max_xyz.cpu().numpy()
    min_x, min_y, min_z = bbox_min_xyz[0]
    max_x, max_y, max_z = bbox_max_xyz[0]
...
def approx_gaussian_bone_volumes(
    ...
    total_joints = tpose_joints.shape[1]

    grid_shape = [grid_size] * 3
    tpose_joints = tpose_joints.cpu().numpy()
    tpose_joints = tpose_joints.astype(np.float32)#(1,24,3)
    tpose_joints =  tpose_joints.reshape(24,3)
